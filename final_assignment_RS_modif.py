...
def hausdorffDistance(metabolite,list_assignment):
	"""
	...
	global tol_H
	list_distance_h = []
	for key_tocsy,peak_tocsy in enumerate(metabolite.tocsy_pattern):
		cs_theo_1 = float(peak_tocsy[0])
		cs_theo_2 = float(peak_tocsy[1])
		min_local = 10000.0 # default value for Hausdorff distance
		for peak_exp in list_assignment[key_tocsy]:
			cs_exp_1 = float(peak_exp[0][0])
			cs_exp_2 = float(peak_exp[0][1])
	...
	while nb_peaks_hausdorff<len(list_distance_h) and list_distance_h[nb_peaks_hausdorff] >= 100.0 :
