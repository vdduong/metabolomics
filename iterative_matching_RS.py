...
def assignment(metabolite, peak_list):
	...
	for item_tocsy in metabolite.tocsy_pattern:
		cs_theor_1 = float(item_tocsy[0])
		cs_theor_2 = float(item_tocsy[1])
		list_assignment.append(list())
		for item_peak_list in peak_list:
			cs_exp_1 = float(item_peak_list[0])
			cs_exp_2 = float(item_peak_list[1])
			if abs(cs_exp_1 - cs_theor_1) <= tol_H and abs(cs_exp_2 - cs_theor_2) <= tol_H :
				proba = math.exp(-0.5*(cs_exp_1 - cs_theor_1)**2/ tol_H**2)*math.exp(-0.5*(cs_exp_2 - cs_theor_2)**2/ tol_H**2)
				proba = float("%.4f"%proba)
				list_assignment[-1].append((item_peak_list, proba))
...
def peaks_found(list_assignment) :
	"""
	...
	for item in list_assignment:
		if len(item)>0:
			nb_peaks_found+=1
#CAUTION : multiple assignment must be taken into account, but set.intersection fonction works only if a specific experimental peak is matched with all theoretical peaks from tocsy_pattern, with the same probability. One solution would be to match each experimental point with the nearest tocsy peak, and then to calculate the number of tocsy peaks mentionned.
...
def iterative_matching(metabolite, peak_list):
	...
	while nb_cycle <= 7 :
		...
		if len(metabolite.tocsy_pattern)==len_initial and len(metabolite.tocsy_pattern) > 1 and float(nb_peaks_found)/float(len(metabolite.tocsy_pattern)) >= 0.7 :
				# more than 70% of peaks are assigned then we continue 
			time_initial=time.time()
			hausdorff, nb_peaks_hausdorff = hausdorffDistance(metabolite, list_assignment)
